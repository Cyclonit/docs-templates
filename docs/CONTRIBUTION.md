...
1. Create the pull request.  
   ...
   1. Name the pull request according to our [naming scheme for pull requests](#naming-scheme-1) and fill out the template.
   1.
...
Try keeping the number and complexity of changes in each commit at a level that is manageable during a code review. A branch may contain as many commits as necessary, thus consider splitting a large number of changes across multiple commits.
...
- We use [markdownlint](https://marketplace.visualstudio.com/items?itemName=davidanson.vscode-markdownlint) to format all of our markdown. Refer to [.markdownlint.json](/.markdownlint.json) for our configuration.
- We use `<br />` to insert one additional line of whitespace before level 2 headings (except the first) to improve readability.
- When creating numbered lists, we only use the index `1.` as this simplifies ommitting/adding elements in templates dynamically. Markdown viewers automatically number the elements in a list correctly.
